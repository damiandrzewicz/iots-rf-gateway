...
uint8_t YA_FSM::AddState(const char* name, uint32_t maxTime, uint32_t minTime,
					action_cb onEntering, action_cb onState, action_cb onLeaving, void* context){
...
uint8_t YA_FSM::AddState(const char* name, uint32_t maxTime, 				 	
					action_cb onEntering, action_cb onState, action_cb onLeaving, void* context)
{
	return AddState(name, maxTime, 0, onEntering, onState, onLeaving, context);
}

uint8_t YA_FSM::AddState(const char* name, 				 	
					action_cb onEntering, action_cb onState, action_cb onLeaving, void* context)
{
	return AddState(name, 0, 0, onEntering, onState, onLeaving, context);
}


uint8_t YA_FSM::AddTransition(uint8_t inputState, uint8_t outputState, condition_cb condition, void* context){
	...
	for(FSM_Transition* actualtr = _firstTransition; actualtr != nullptr; actualtr = actualtr->nextTransition) {
		if(actualtr->InputState == _currentState->index){
			...
			if(actualtr->Condition == nullptr)
				...
				_trigger = actualtr->Condition(actualtr->Context);
			...
			if (_trigger){
				...
				if(_currentState->OnLeaving != nullptr)
					_currentState->OnLeaving(_currentState->Context);
				...
				_currentState->result = FSM_State::Result::InWork;
				_currentState->step = 1;

				if(_currentState->OnEntering != nullptr)
					_currentState->OnEntering(_currentState->Context);
				return true;
			}
		}
	}
	if(_currentState->OnState != nullptr)
			_currentState->OnState(_currentState->Context);
...
void YA_FSM::initVariables(){
	for(uint8_t i=0; i<_numStates; i++){
		AddState("", 0, nullptr, nullptr, nullptr, nullptr);
	}
}
